...
export default function AppLogoBox(props) {
    ...
    const handleGoToApp = (props) => {
      ...
      console.log("여기야", router);
      (props.target.id === 1) ? (router.push('/user/register/employee')):(router.push('/'));
    }
  
    return (
        <div className={styles.flex}>
        {appData.map(data =>
            <div className={styles.go_to_app_rectangle} id={data.id} key={data.id} onClick={handleGoToApp}>
                    <img src={data.src}
                        srcSet={data.srcSet}
                    />
                <span id={data.id} key={data.id} className={styles.app_text_logo}>{data.textLogo}</span>
            </div>
        )}
        </div>
  )
