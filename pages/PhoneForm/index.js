...
import { MY_LOCATION_CALL, SUGGESTING, RESERVATION_CONFIRMATION, DISPATCHING, RETURNED }
        ...
        <div className={styles.shadow_phone_form}>
            ...
            <div>
                {rentalProcessList.map(el => 
                    <span style={el.id === 1 ? {color:'black', borderBottom:'1px solid #1877f2'}:{color:'#bdbdbd'}} key={el.id} className={styles.rental_process_list}>
                        {el.list}
                    </span>
                )}
            </div>
            {callData.map(el => 
                <div key={el.id} className={styles.call_container}>
                    ...
                    <div>
                        <div className={styles.company_suggestion}>
                            공업사 요청
                        </div>
                        <div>
